  ...
  const openMenu = (event, type, value) => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if((menuOpen && menuRef.current && !menuRef.current.contains(event.target)) || (profileOpen && profileRef.current && !profileRef.current.contains(event.target))){
                dispatch({
                    type: 'ChangeMenuOpen',
                    payload: false,
                })
      ...
      document.removeEventListener("click", handleClickOutside);
            ...
            {windowWidth <= 1100 ?
                <div className={style.containerMenu}>
                    <button className={menuOpen ? style.menuOpen : style.menuClose} onClick={(event) => openMenu(event, 'ChangeMenuOpen', !menuOpen)}>
                        {menuOpen?
                            <CgClose />
                            :
                            <CgMenuCheese />}
            ...
            {isLoged ? 
                <>
                    <button className={profileOpen ? style.profileIconOpen : style.profileIconClose} onClick={(event) => openMenu(event, 'ChangeProfileOpen', !profileOpen)}>
                        <FaCircleUser />
                    </button>
                    {profileOpen ?
                ...
                :
                <>
                    <button className={style.entreCadastre} onClick={(event) => openMenu(event, 'ChangeProfileOpen', !profileOpen)}>Comece agora</button>
                    {profileOpen ?
                        <div className={style.profileMenu} ref={profileRef}>
